            ...
            <div className="box-border w-[100%] pl-[30px] pr-[40px]">
                <ul className="overflow-y-auto flex min-[650px]:flex-row min-[650px]:flex-wrap max-[650px]:flex-col list-none dropbox">
                    {galleryitems && galleryitems.map((item, index) => (
                        <GalleryItem setDetailView={setDetailView} sw={width} key={index} price={item.price} listed={item.listed} image={item.image} token_id={item.token_id} description={item.description} owner={item.owner} />
                    ))}
                </ul>
            </div>
        </section>
